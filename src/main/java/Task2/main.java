...
public class main {
    public static void main(String[] args) {
        ...
        words=Arrays.stream(words).map(s->s.toLowerCase())
                .collect(Collectors.toList())
                .toArray(new String[words.length]);

        var wordsSet = new TreeSet<String>(Arrays.stream(words).toList());

        System.out.printf("Amount of unique words: %d",wordsSet.size());
    }
}
